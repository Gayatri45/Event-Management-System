...
<title>One Stop | Audiences</title>
...
<body>
					...
					<div class="row d-flex align-items-center">
						...
						<div
							...
							<a class="btn btn-success btn-sm m-b-10 m-l-5"
								onclick="addAudienceFunc()"><i class="ti-plus"></i> Add
								Audience</a>
						</div>
					</div>
				</div>
								...
								<div class="card-body">
									<div class="row d-flex align-items-center">
										...
										<div class="col-md-2">
											<div class="form-group row m-b-0">
												<div class="col-md-12">
													<button type="button" onclick="loadAudience()"
	...
	<!-- Add Audience -->
	<div class="modal fade" id="audienceModal" tabindex="-1"
		...
		<div class="modal-dialog modal-dialog-centered modal-md">
			<div class="modal-content">
				...
				<form id="saveAudience" name="saveAudience"
						 ...
						 <input type="hidden" id="companyNameDetails" name="companyName">
							...
							<div class="col-md-12">
								<div class="form-group">
									<label>Audience Name <span class="text-danger">*</span></label>  
									<input type="text" class="form-control" name="audienceName" id="audienceName"
                                                        placeholder=" ">
								</div>
							</div>
						</div>
					...
					<div class="modal-footer">
						...
						<button type="submit" class="btn btn-success"
							onclick="return validateAudience()">Save</button>
					</div>
				</form>
			</div>
		</div>
	...
	<!-- Update Audience -->
	<div class="modal fade" id="audienceModalEdit" tabindex="-1"
		aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered modal-md">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">Update Audience</h5>
					<button type="button" class="close" data-dismiss="modal"
						aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				...
				<form id="editSaveAudience" name="editSaveAudience"
					enctype="multipart/form-data">
					<div class="modal-body">
						<div class="row">
							...
							<input type="hidden" id="editBranchName" name="branchName"> <input
								...
								<input type="hidden" id="editAudienceId" name="audienceId">

							<div class="col-md-12">
								<div class="form-group">
									<label>Audience Name <span class="text-danger">*</span></label>  
									<input type="text" class="form-control" name="audienceName" id="editAudienceName"
                                                        placeholder=" ">
								</div>
							</div>
						</div>
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary"
							data-dismiss="modal">Close</button>
						<button type="submit" class="btn btn-success"
							onclick="return validateAudience1()">Save</button>
					</div>
				</form>
			</div>
		</div>
				...
				$.ajax({
					...
					success : function(branchMap) {
						...
						for (var i = 0; i < parsedData.length; i++) {
							...
							//$("#branchId").val(parsedData.branchId); //for 1
	...
	<script>
		...
		$('#saveAudience').submit(function (event) { 
	        event.preventDefault();                 
	        var form = document.getElementById('saveAudience'); 
	        var formData = new FormData(form); 
	       
	        $.ajax({ 
	            url: '/organizer/categoryAndAudience/saveAudience', 
	            method: 'POST', 
	            data: formData, 
	            processData: false, 
	            contentType: false, 
	...
	<script>
		function loadAudience() {
			...
			if (companyId == "0") {
				...
				$.ajax({
					url : '/organizer/categoryAndAudience/getAudienceTableList', // JSP page to fetch data
					type : 'POST',
					data : {
						companyId : companyId,
						branchId : branchId
	...
	<script>	
		function validateAudience(){
			...
			var audienceName = $("#audienceName").val();
			...
			if (audienceName == "" || audienceName == null) {
				swal("Oops...", "Please Enter Audience Name !!", "error");
				$('#audienceName').focus();
				return false;
			}
		}
	...
	<script>
		function editAudienceFunc(audienceId) {
			...
			if (branchId == "0" || branchId == "") {
				...
				$.ajax({
					type : 'POST',
					url : "/organizer/categoryAndAudience/getAudienceById",
					data : {
					...
					success : function(audience) {
						...
						var parsedData = JSON.parse(audience);
						console.log(parsedData);	
						$('#audienceModalEdit').modal('toggle');
						...
						$("#editAudienceId").val(audienceId);
						$("#editAudienceName").val(parsedData.audienceName);
	...
	<script>	
		function validateAudience1(){
			
			var companyId = $("#editCompanyId").val();
			var branchId = $("#editBranchId").val();
			var audienceName = $("#editAudienceName").val();			
	...
	<script>
		...
		$('#editSaveAudience').submit(function (event) { 
	        event.preventDefault();                 
	        var form = document.getElementById('editSaveAudience'); 
	        var formData = new FormData(form); 
