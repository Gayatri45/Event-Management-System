...
(function ($) {
  ...
  $.fn.itmimageupload = function (methodOrOptions) {
  ...
  $.fn.itmimageupload.defaultOptions = {
  ...
  function init(givenOptions) {
    options = $.extend({}, $.fn.itmimageupload.defaultOptions, givenOptions);
  ...
  function getAlertHtml(message) {
    var html = [];
    html.push('<div class="alert alert-warning alert-dismissible radius-0">');
  ...
  function getImageThumbnailHtml(src) {
    return (
      '<img src="' +
      src +
      '" alt="Image preview" class="thumbnail d-block" style="max-width: ' +
      options.maxWidth +
      "px; max-height: " +
      options.maxHeight +
      'px">'
  ...
  function resetFileTab($fileTab) {
    ...
    $fileTab.find(".btn span").text("Upload Image");
  ...
  function submitImageFile($fileTab) {
    ...
    $fileTab.find(".alert").remove();
    $fileTab.find("img").remove();
    $browseFileButton.find("span").text("Upload Image");
  ...
  function submitImageUrl($urlTab) {
    ...
    isValidImageUrl(url, function (isValid, message) {
      if (isValid) {
        ...
        // Show thumbnail and remove button.
        $(getImageThumbnailHtml(url)).insertAfter(
          $submitUrlButton.closest(".input-group")
        );
