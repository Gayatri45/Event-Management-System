...
+function ($) {
    ...
    DropdownSelect.prototype.updateDisplay = function( mode, selected ) {
        if( mode == 'select') {
            ...
            this.structure.$display.html('<span class="text">' + selected.html()).addClass(this.options.filled);
        }

        if( mode == 'clear') {
            this.structure.$clear.hide();
            $('input[name="' + this.structure.$section + '"]').val('');
            this.structure.$display.html('<span class="placeholder">' + this.structure.$placeholder.html()).removeClass(this.options.filled);
        }
    }
